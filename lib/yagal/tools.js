var yagal_tools = (function() {
  ...
  function mutPoint(subFunc, individual) {
    // Mutates (75% chance) or kills (25%) a single random action in the sequence
    var point = randInt(individual.length);
    if (Math.random() <= 0.75) {
      individual[point] = subFunc();
    }
    else {
      individual.splice(point, 1);
    }
    return [individual];
  }
